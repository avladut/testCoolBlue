...
class ProductListViewModel {
    ...
    init(httpManager:HTTPRequestAPI) {
    ...
    public func search() {
        guard let httpManagerUnwr = httpManager ,let searchQueryUnwr = strSearchQuery else {
            return
        }
        ...
        httpManagerUnwr.searchFor(searchQueryUnwr, page: intLastSearchedPage) {[weak self] (dictResponse, error) in
